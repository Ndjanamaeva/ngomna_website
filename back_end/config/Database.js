...
sequelize.sync({ force: true }) // This will recreate the tables, use { alter: true } to update existing ones
  .then(async () => {
    ...
    const featuresMenu = await Menu.create({
      title: 'features',
      menuItems: [
        { label: 'payslips', pageId: pages[0].id },
        { label: 'information', pageId: pages[1].id },
        { label: 'notifications', pageId: pages[2].id },
        { label: 'census', pageId: pages[3].id },
        { label: 'messaging', pageId: pages[4].id },
        { label: 'children', pageId: pages[5].id },
        { label: 'security', pageId: pages[6].id },
        { label: 'OTP', pageId: pages[7].id },
        { label: 'DGI', pageId: pages[8].id }
      ]
    }, {
